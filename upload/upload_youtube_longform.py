...
def get_latest_video():
    ...
    if not longform_dir.exists():
        print("âŒ No longform_videos directory found")
        return None
    ...
    if not dirs:
        print("âŒ No video directories found")
        return None

    latest_dir = dirs[0]
    print(f"ðŸ“ Found latest video: {latest_dir.name}")
    ...
    if not video_file.exists():
        print(f"âŒ Video file not found: {video_file}")
        return None
    ...
    if (latest_dir / "thumbnail.jpg").exists():
        ...
        print(f"ðŸ–¼ï¸  Using custom thumbnail: thumbnail.jpg")
    else:
        phrase_images = sorted(latest_dir.glob("phrase_*.jpg"))
        if phrase_images:
            thumbnail_file = phrase_images[0]
            print(f"ðŸ–¼ï¸  Using phrase image as thumbnail: {thumbnail_file.name}")
...
def parse_upload_info(info_file):
    ...
    try:
        ...
        if "ðŸ“ SELECTED TITLE" in content:
            title_section = content.split("ðŸ“ SELECTED TITLE")[1]
            if "--------------------------------------------------------------------------------" in title_section:
                title = title_section.split("--------------------------------------------------------------------------------")[1].strip()
                title = title.split("\n")[0].strip()
        
        # Find description section
        if "ðŸ“„ VIDEO DESCRIPTION:" in content:
            desc_section = content.split("ðŸ“„ VIDEO DESCRIPTION:")[1]
            if "--------------------------------------------------------------------------------" in desc_section:
                description = desc_section.split("--------------------------------------------------------------------------------")[1].strip()
    except Exception as e:
        print(f"âš ï¸  Error parsing upload info: {e}")
...
def upload_to_youtube(video_info):
    ...
    try:
        ...
        print("âŒ YouTube API libraries not installed.")
    ...
    if YT_REFRESH_TOKEN:
        ...
        try:
            ...
            print(f"âš ï¸  Refresh token failed: {e}")
            credentials = None
    else:
        credentials = None
    
    if not credentials:
        print("âŒ No valid YouTube credentials found.")
    ...
    try:
        ...
        if video_info.get("thumbnail") and video_info["thumbnail"].exists():
            print(f"\nðŸ–¼ï¸  Uploading custom thumbnail...")
            try:
        ...
        print(f"\nðŸ“„ Upload result saved to: {result_file}")
        
        return True
        
    except HttpError as e:
        print(f"\nâŒ YouTube upload failed: {e}")
        if "quotaExceeded" in str(e):
            print("   âš ï¸  Daily upload quota exceeded. Try again tomorrow.")
        return False
    except Exception as e:
...
def main():
    print("\n" + "="*80)
    print("\U0001F1E9\U0001F1F0 VELOCITY DANISH - YOUTUBE LONG-FORM UPLOADER \U0001F1E9\U0001F1F0")
    print("="*80)
    
    # Find latest video
    video_info = get_latest_video()
    if not video_info:
        print("\nâŒ No video found to upload")
        print("   Run youtube_longform_automation.py first to generate a video")
        return False
    
    print(f"\nðŸ“ Video Directory: {video_info['dir']}")
    print(f"   ðŸŽ¬ Video: {video_info['video'].name}")
    print(f"   ðŸ–¼ï¸  Thumbnail: {video_info['thumbnail'].name if video_info.get('thumbnail') else 'Not found'}")
    
    # Confirm upload
    print(f"\nâœ… Videos will be uploaded as PUBLIC")
    print(f"   They will be visible to everyone immediately")
    
    # Check if running in GitHub Actions (non-interactive)
    if os.getenv("GITHUB_ACTIONS") == "true":
        print(f"\nðŸ¤– Running in GitHub Actions - auto-confirming upload")
        confirm = True
    else:
